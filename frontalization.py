...
def half_flip(img):  
    ...
    try:
        ...
        output_list.append((img_ori, None))          
        ...
        if landmark_face is None:   
            ...
            output_list.append((flip_output, None))    
...
def face_no_background(img):
    ...
    _, landmark_face = landmark_wajah(img_roll)      
    if landmark_face is None:                        
        return None
    else:
        dua_D = []                                   
        ...
        poin_wajah_full = list_poin_wajah(dua_D)
        warpp_full = images_warping(img_roll, poin_wajah_full, poin_wajah_full)
        face_ori = potong_area_(warpp_full, poin_wajah_full)
        face_ori = cv2.resize(face_ori, (128, 128))
        return face_ori